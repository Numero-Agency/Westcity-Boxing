  ...
  public function post_updated_messages( $messages ) {
    ...
    if ( $public ) {
      $messages[ $slug ][1] = sprintf(
        __( '%1$s updated. <a href="%2$s">View %3$s</a>', 'memberpress-coachkit' ),
        $singular_name,
        esc_url( get_permalink( $post_ID ) ),
        $singular_name
      );
    } else {
      $messages[ $slug ][1] = sprintf( __( '%1$s updated.', 'memberpress-coachkit' ), $singular_name );
    }

    $messages[ $slug ][2] = __( 'Custom field updated.', 'memberpress-coachkit' );
    $messages[ $slug ][3] = __( 'Custom field deleted.', 'memberpress-coachkit' );
    $messages[ $slug ][4] = sprintf( __( '%s updated.', 'memberpress-coachkit' ), $singular_name );
    $messages[ $slug ][5] = isset( $_GET['revision'] ) ? sprintf( __( '%1$s restored to revision from %2$s', 'memberpress-coachkit' ), $singular_name, wp_post_revision_title( (int) $_GET['revision'], false ) ) : false;

    if ( $public ) {
      $messages[ $slug ][6] = sprintf(
        __( '%1$s published. <a href="%2$s">View %3$s</a>', 'memberpress-coachkit' ),
        $singular_name,
        esc_url( get_permalink( $post_ID ) ),
        $singular_name
      );
    } else {
      $messages[ $slug ][6] = sprintf( __( '%1$s published.', 'memberpress-coachkit' ), $singular_name );
    }

    $messages[ $slug ][7] = sprintf( __( '%s saved.', 'memberpress-coachkit' ), $singular_name );

    if ( $public ) {
      $messages[ $slug ][8]  = sprintf(
        __( '%1$s submitted. <a target="_blank" href="%2$s">Preview %3$s</a>', 'memberpress-coachkit' ),
        $singular_name,
        esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ),
        $singular_name
      );
      $messages[ $slug ][9]  = sprintf(
        __( '%1$s scheduled for: <strong>%2$s</strong>. <a target="_blank" href="%3$s">Preview %4$s</a>', 'memberpress-coachkit' ),
        $singular_name,
      ...
      $messages[ $slug ][10] = sprintf(
        __( '%1$s draft updated. <a target="_blank" href="%2$s">Preview %3$s</a>', 'memberpress-coachkit' ),
        $singular_name,
        esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ),
        $singular_name
      );
    } else {
      $messages[ $slug ][8]  = sprintf( __( '%s submitted.', 'memberpress-coachkit' ), $singular_name );
      $messages[ $slug ][9]  = sprintf(
        __( '%1$s scheduled for: <strong>%2$s</strong>.', 'memberpress-coachkit' ),
        $singular_name,
        date_i18n( 'M j, Y @ G:i', strtotime( $post->post_date ), true )
      );
      $messages[ $slug ][10] = sprintf( __( '%s draft updated.', 'memberpress-coachkit' ), $singular_name );
    }
