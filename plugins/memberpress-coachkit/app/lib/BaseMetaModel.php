...
abstract class BaseMetaModel extends BaseModel {
  ...
  public function get_meta( $meta_key, $single = false ) {
    ...
    return $db->get_metadata( $db->{$this->meta_table}, "{$this->object_type}_id", $this->id, $meta_key, $single );
  }
   ...
   */
  public function add_meta( $meta_key, $meta_value, $unique_key = false, $unique_value = false ) {
    $db = lib\Db::fetch();

    return $db->add_metadata( $db->{$this->meta_table}, "{$this->object_type}_id", $this->id, $meta_key, $meta_value, $unique_key, $unique_value );
  }
  ...
  private function object_type() {
    ...
    return strtolower( str_replace( 'memberpress\coachkit\models\\', '', $mepr_class ) );
  }
}
