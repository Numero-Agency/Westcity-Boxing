...
/** Ctrls in MemberPress Coaching are all singletons, so we can
 * use this factory to churn out objects for us.
 */
class CtrlFactory {
  public static function fetch( $class, $args = array( 'path' => base\CTRLS_PATH ) ) {
    static $objs;
    ...
    if ( ! class_exists( $class ) ) {
      throw new \Exception( sprintf( __( 'Ctrl: %s wasn\'t found', 'memberpress-coachkit' ), $class ) );
    }

    // We'll let the autoloader in main.php
    // handle including files containing these classes
    $r   = new \ReflectionClass( $class );
    $obj = $r->newInstanceArgs( array_values( $args ) );
