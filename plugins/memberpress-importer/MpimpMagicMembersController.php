...
class MpimpMagicMembersController {
  ...
  /** We currently support only two of the same payment methods that
  ...
  public static function migrate_coupons($packs, $map) {
    ...
    foreach($mgm_coupons as $mgm_coupon) {
      // Don't migrate coupons that are incompatible.
      if(!is_numeric(preg_replace('/\%/','',$mgm_coupon->value)))
        continue;

      $c = new MeprCoupon();
      $c->post_title    = $mgm_coupon->name;
      $c->should_expire = !empty($mgm_coupon->expire_dt);

      if(empty($mgm_coupon->expire_dt)) {
        $c->expires_on=0;
      }
      else {
        $d = (object)date_parse( $mgm_coupon->expire_dt );
        $c->expires_on = MeprUtils::make_ts_date( $d->month, $d->day, $d->year );
      }

      $c->usage_count   = empty($mgm_coupon->used_count)?0:$mgm_coupon->used_count;
      $c->usage_amount  = empty($mgm_coupon->use_limit)?0:$mgm_coupon->use_limit;
      $c->discount_type = preg_match('/\%/',$mgm_coupon->value)?'percent':'dollar';
      $c->discount_amount = number_format(preg_replace('/\%/','',$mgm_coupon->value),2);
