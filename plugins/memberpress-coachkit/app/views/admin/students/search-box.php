<span class="filter-by">
  ...
  <select class="mepr_filter_field" id="r_coach">
    <option value="all" <?php selected( $r_coach, false ); ?>><?php printf( esc_html__( 'All %s', 'memberpress-coachkit' ), $label_for_coaches); ?></option>
    <?php foreach ( $coaches as $coach ) : ?>
      <option value="<?php echo absint( $coach->ID ); ?>" <?php selected( $coach->ID, $r_coach ); ?>><?php echo esc_html( $coach->name ); ?></option>
    <?php endforeach; ?>
  </select>

  <?php if ( $groups ) : ?>
  <select class="mepr_filter_field" id="r_group">
    <option value="all" <?php selected( $r_group, false ); ?>><?php esc_html_e( 'All Cohorts', 'memberpress-coachkit' ); ?></option>
    <?php foreach ( $groups as $group ) : ?>
      <option value="<?php echo esc_attr( $group->id ); ?>" <?php selected( $group->id, $r_group ); ?>><?php echo esc_html( $group->title ); ?></option>
    <?php endforeach; ?>
  </select>
  <?php endif; ?>
