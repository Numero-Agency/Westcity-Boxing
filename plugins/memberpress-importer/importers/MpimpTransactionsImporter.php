...
class MpimpTransactionsImporter extends MpimpBaseImporter {
  public function form() {
    ...
    ?>
    <input type="checkbox" name="args[welcome]" />&nbsp; <?php _e('Send a Welcome Email with each new transaction', 'memberpress-importer'); ?>
    <?php
  }
  ...
  public function import($row,$args) {
      ...
      switch($col) {
        ...
        case "user_email":
          ...
          if($col == "username") {
            ...
            if(!$usr->ID)
              throw new Exception(sprintf(__('username=%1$s wasn\'t found so couldn\'t create transaction', 'memberpress-importer'), $cell));
          }
          else {
            $usr->load_user_data_by_email($cell);
            if(!$usr->ID)
              throw new Exception(sprintf(__('email=%1$s wasn\'t found so couldn\'t create transaction', 'memberpress-importer'), $cell));
          }
    ...
    if($txn_id) {
      ...
      if($updating_txn) {
        $txn_message = sprintf(__('Transaction with trans_num = %s was found with an id = %s and was updated successfully', 'memberpress-importer'), $txn->trans_num, $txn_id);
      }
      else {
        $txn_message = sprintf(__('Transaction (id = %s) was created successfully', 'memberpress-importer'), $txn_id);
      }

      return $txn_message;
    }
    else {
      throw new Exception(__('Transaction failed to be created or updated', 'memberpress-importer'));
    }
  }
}
