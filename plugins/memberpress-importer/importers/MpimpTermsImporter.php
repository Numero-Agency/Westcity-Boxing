...
class MpimpTermsImporter extends MpimpBaseImporter {
  ...
  public function import($row,$args) {
    ...
    if( $t = term_exists( $term, $taxonomy, $params['parent'] ) ) {
      ...
      if( !is_wp_error( $ids ) ) {
        return sprintf( __('Term (term=%1$s, term_id=%2$s) already existed and was updated successfully', 'memberpress-importer'), $term, $ids['term_id'] );
      }
      else {
        throw new Exception(__('Term existed and failed to be updated', 'memberpress-importer'));
      }
    }
    else {
      $ids = wp_insert_term( $term, $taxonomy, $params );
      if( !is_wp_error( $ids ) ) {
        return sprintf( __('Term (term=%1$s, term_id=%2$s) was created successfully', 'memberpress-importer'), $term, $ids['term_id'] );
      }
      else {
        throw new Exception(__('Term failed to be created', 'memberpress-importer'));
      }
    }
  }
}
