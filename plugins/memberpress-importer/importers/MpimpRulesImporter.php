...
class MpimpRulesImporter extends MpimpBaseImporter {
  ...
  public function import($row,$args) {
          ...
          case "expire_trigger":
            ...
            if( is_numeric($cell) )
              ...
              if($cell != 'registers' && $cell != 'fixed' && $cell != 'rule-products')
                throw new Exception( sprintf( __('%1$s must be the word "registers", "fixed", "rule-products" or a valid product_id', 'memberpress-importer'), $col ) );
            }
    ...
    if( $rule_id = $rule->store() ) {
      $success_msg = sprintf(__('Rule (ID = %d) was created successfully.', 'memberpress-importer'), $rule_id);
      if(!$this->legacy_import)
        $success_msg .= __(" Don't forget to import or create AccessConditions", 'memberpress-importer');
      return $success_msg;
    }
    else
      throw new Exception(__('Rule failed to be created', 'memberpress-importer'));
  }
}
