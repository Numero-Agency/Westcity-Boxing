...
class MpimpSubscriptionsImporter extends MpimpBaseImporter {
  ...
  public function import($row,$args) {
    ...
    if($sub_id) {
      if($updating_sub) {
        $sub_message = sprintf(__('Subscription with sub_num = %s was found with an id = %s and was updated successfully', 'memberpress-importer'), $sub->subscr_id, $sub_id);
      }
      else {
        $sub_message = sprintf(__('Subscription (id = %s) was created successfully', 'memberpress-importer'), $sub_id);
      }

      return $sub_message;
    }
    else {
      throw new Exception(__('Subscription failed to be created', 'memberpress-importer'));
    }
  }
}
