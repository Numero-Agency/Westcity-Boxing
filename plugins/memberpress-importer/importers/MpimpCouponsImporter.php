...
class MpimpCouponsImporter extends MpimpBaseImporter {
  ...
  public function import($row,$args) {
    ...
    if( $coupon_id = $coupon->store() )
      return sprintf(__('Coupon (ID = %s) was %s successfully', 'memberpress-importer'), $coupon_id, true === $override ? __('updated', 'memberpress-importer') : __('created', 'memberpress-importer'));
    else
      throw new Exception(__('Coupon failed to be created', 'memberpress-importer'));
  }
}
