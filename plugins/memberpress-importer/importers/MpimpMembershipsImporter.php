...
class MpimpMembershipsImporter extends MpimpBaseImporter {
  ...
  public function import($row,$args) {
    ...
    if( $product_id = $product->store() )
      return sprintf(__('Membership (ID = %d) was created successfully', 'memberpress-importer'), $product_id);
    else
      throw new Exception(__('Membership failed to be created', 'memberpress-importer'));
  }
}
