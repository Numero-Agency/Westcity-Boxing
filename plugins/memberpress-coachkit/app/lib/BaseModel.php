...
abstract class BaseModel {
  ...
  public function get_json() {
    return wp_json_encode( $this->get_values() );
  }
  ...
  protected function validate_not_null( $var, $field = '' ) {
    if ( is_null( $var ) ) {
      throw new CreateException( sprintf( __( '%s must not be empty', 'memberpress-coachkit' ), $field ) );
    }
  }
  ...
  protected function validate_is_bool( $var, $field = '' ) {
    if ( ! is_bool( $var ) && $var != 0 && $var != 1 ) {
      throw new CreateException( sprintf( __( '%s must be true or false', 'memberpress-coachkit' ), $field ) );
    }
  }

  protected function validate_is_array( $var, $field = '' ) {
    if ( ! is_array( $var ) ) {
      throw new CreateException( sprintf( __( '%s must be an array', 'memberpress-coachkit' ), $field ) );
    }
  }

  protected function validate_is_in_array( $var, $lookup, $field = '' ) {
    if ( is_array( $lookup ) && ! in_array( $var, $lookup ) ) {
      throw new CreateException( sprintf( __( '%1$s must be %2$s NOT %3$s', 'memberpress-coachkit' ), $field, implode( ' ' . __( 'or', 'memberpress-coachkit' ) . ' ', $lookup ), $var ) );
    }
  }

  protected function validate_is_url( $var, $field = '' ) {
    if ( ! Utils::is_url( $var ) ) {
      throw new CreateException( sprintf( __( '%1$s (%2$s) must be a valid url', 'memberpress-coachkit' ), $field, $var ) );
    }
  }

  protected function validate_is_currency( $var, $min = 0.00, $max = null, $field = '' ) {
    if ( ! is_numeric( $var ) || $var < $min || ( ! is_null( $max ) && $var > $max ) ) {
      throw new CreateException( sprintf( __( '%1$s (%2$s) must be a valid representation of currency', 'memberpress-coachkit' ), $field, $var ) );
    }
  }

  protected function validate_is_numeric( $var, $min = 0, $max = null, $field = '' ) {
    if ( ! is_numeric( $var ) || $var < $min || ( ! is_null( $max ) && $var > $max ) ) {
      throw new CreateException( sprintf( __( '%1$s (%2$s) must be a valid number', 'memberpress-coachkit' ), $field, $var ) );
    }
  }

  protected function validate_is_email( $var, $field = '' ) {
    if ( ! Utils::is_email( $var ) ) {
      throw new CreateException( sprintf( __( '%1$s (%2$s) must be a valid email', 'memberpress-coachkit' ), $field, $var ) );
    }
  }

  protected function validate_is_phone( $var, $field = '' ) {
    if ( ! Utils::is_phone( $var ) ) {
      throw new CreateException( sprintf( __( '%1$s (%2$s) must be a valid phone number', 'memberpress-coachkit' ), $field, $var ) );
    }
  }

  protected function validate_is_ip_addr( $var, $field = '' ) {
    if ( ! Utils::is_ip( $var ) ) {
      throw new CreateException( sprintf( __( '%1$s (%2$s) must be a valid IP Address', 'memberpress-coachkit' ), $field, $var ) );
    }
  }

  protected function validate_is_date( $var, $field = '' ) {
    if ( ! Utils::is_date( $var ) ) {
      throw new CreateException( sprintf( __( '%1$s (%2$s) must be a valid date', 'memberpress-coachkit' ), $field, $var ) );
    }
  }
  ...
  protected function validate_is_timestamp( $var, $field = '' ) {
    if ( empty( $var ) || ! is_numeric( $var ) ) {
      throw new CreateException( sprintf( __( '%1$s (%2$s) must be a valid timestamp', 'memberpress-coachkit' ), $field, $var ) );
    }
  }

  protected function validate_regex( $pattern, $var, $field = '' ) {
    if ( ! preg_match( $pattern, $var ) ) {
      throw new CreateException( sprintf( __( '%1$s (%2$s) must match the regex pattern: %3$s', 'memberpress-coachkit' ), $field, $var, $pattern ) );
    }
  }
   ...
   * Gets one row
   *
   * @param array $args column/value pairs
   * @return Object|null
   */
